...
SELECT ROUND((COUNT(DISTINCT Order_ID) / (SELECT COUNT(DISTINCT Order_ID)
...
SELECT Customer_Name,
    avg_order_value,
   ' DENSE_RANK() OVER (ORDER BY avg_order_value DESC) AS sales_rank'
FROM ( SELECT
Customer_Name,
